...
export class AppUI {
          ...
          <div>
            ...
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <p class="text-sm font-medium text-slate-500 dark:text-slate-400 uppercase tracking-widest">Financial Planning</p>
              </div>
            </div>
          </div>
          ...
          <section class="lg:col-span-5 space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-[2rem] shadow-2xl shadow-slate-200/50 dark:shadow-black/40 border border-gold-100 dark:border-slate-700 p-8 relative overflow-hidden group">
              ...
              <div class="relative z-10 text-center mb-8">
                <p class="text-slate-400 dark:text-slate-500 font-bold uppercase tracking-[0.2em] text-[10px] mb-2">Monthly Installment</p>
              ...
              <div class="grid grid-cols-2 gap-4 relative z-10">
                <div class="p-5 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-700">
                  <p class="text-slate-400 dark:text-slate-500 text-[9px] font-bold uppercase tracking-wider mb-1">Total Interest</p>
                  <h3 id="totalInterestDisplay" class="text-xl font-bold text-slate-900 dark:text-white tabular-nums">₹0</h3>
                  <p id="interestPercentDisplay" class="text-[10px] text-slate-400 mt-1">0% of total</p>
                </div>
                <div class="p-5 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-700">
                  <p class="text-slate-400 dark:text-slate-500 text-[9px] font-bold uppercase tracking-wider mb-1">Total Payable</p>
                  <h3 id="totalAmountDisplay" class="text-xl font-bold text-slate-900 dark:text-white tabular-nums">₹0</h3>
                  <p class="text-[10px] text-slate-400 mt-1">Principal + Int.</p>
                </div>
              </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-700 p-8">
              ...
              <div class="border-t border-slate-100 dark:border-slate-700 pt-8">
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-6 text-center">Projected Balance Reduction</p>
                <div class="h-40">
    ...
    renderInputGroup(id, label, min, max, step, suffix, inputId) {
      ...
      <div class="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700 transition-all hover:border-gold-200 dark:hover:border-gold-900/50">
        <div class="flex justify-between items-center mb-5">
          <label for="${inputId}" class="text-slate-400 dark:text-slate-500 font-bold text-[10px] uppercase tracking-widest">
            ${label}
          </label>
          <div class="flex items-center bg-slate-50 dark:bg-slate-900/50 rounded-xl px-4 py-2 border border-slate-200 dark:border-slate-700 focus-within:ring-2 focus-within:ring-gold-500/50 transition-all">
            <input
              ...
              type="number"
              value="${this.state[id]}"
              class="w-24 bg-transparent text-right font-black text-slate-900 dark:text-white outline-none text-base"
              placeholder="0"
            />
        ...
        <div class="flex justify-between mt-3 text-[10px] text-slate-300 dark:text-slate-600 font-bold uppercase tracking-tighter">
          <span>${this.formatCompact(min)}</span>
          <span>${this.formatCompact(max)}</span>
        </div>
      </div>
    `;
    }
        ...
        <div class="space-y-2">
          <div class="flex justify-between text-[11px]">
            ...
            <span class="${comp.diffEMI > 0 ? 'text-rose-500' : 'text-emerald-500'} font-bold tabular-nums">
              ${comp.diffEMI > 0 ? '+' : ''}${this.formatCompact(comp.diffEMI)}
            </span>
          </div>
          <div class="flex justify-between text-[11px]">
            ...
            <span class="${comp.diffInterest > 0 ? 'text-rose-500' : 'text-emerald-500'} font-bold tabular-nums">
              ${comp.diffInterest > 0 ? '+' : ''}${this.formatCompact(comp.diffInterest)}
            </span>
          </div>
        </div>
      </button>
    ...
    renderTable(schedule) {
      ...
      <table class="w-full text-xs text-left">
        <thead class="text-[10px] text-slate-400 uppercase bg-slate-50/80 dark:bg-slate-900/80 sticky top-0 z-10 backdrop-blur-md">
          <tr>
        ...
        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
          ${schedule.map((row) => `
            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors">
              ...
              <td class="px-6 py-3 text-right text-emerald-600 font-mono">${this.formatCompact(row.principal)}</td>
              <td class="px-6 py-3 text-right text-gold-500 font-mono">${this.formatCompact(row.interest)}</td>
              <td class="px-6 py-3 text-right text-slate-400 font-mono">${this.formatCompact(row.balance)}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
    ...
    async sharePlan() {
        const { emi, totalInterest } = LoanCalculator.calculate(this.state.amount, this.state.rate, this.state.tenureYears);
    ...
    copyPlan() {
        const { emi, totalInterest } = LoanCalculator.calculate(this.state.amount, this.state.rate, this.state.tenureYears);
        const text = `EMI: ${this.formatCurrency(emi)} | Total Interest: ${this.formatCurrency(totalInterest)} | Duration: ${this.state.tenureYears} Years`;
        this.copyToClipboard(text);
    }
